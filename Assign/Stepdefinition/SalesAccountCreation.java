...
public class SalesAccountCreation {
	
	
	public ChromeDriver driver ; 
	
	
	  @Given("Launch the browser and Load the salesforce {string}") 
	  public void launch_the_browser_and_load_the_salesforce_url(String url) { 
		  driver = new ChromeDriver() ; 
		  driver.manage().window().maximize(); driver.get(url);
		  driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(8)) ; }
	  
	  @When("Enter the salesforce username as {string}") 
	  public void enter_the_salesforce_username_as(String uname) {
	  driver.findElement(By.id("username")).sendKeys(uname) ;
	  }
	  
	  @And("Enter the salesforce password as {string}") 
	  public void enter_the_salesforce_password_as(String pass) {
